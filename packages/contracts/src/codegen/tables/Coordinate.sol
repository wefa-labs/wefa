...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("Coordinate")))
);
ResourceId constant CoordinateTableId = _tableId;
...
library Coordinate {
   ...
   */
  function get(bytes32 key) internal view returns (CoordinateData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 key) internal view returns (CoordinateData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function set(bytes32 key, CoordinateData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 key, CoordinateData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y, _table.z);
  ...
  function decode(
    ...
    PackedCounter,
    bytes memory
  ) internal pure returns (CoordinateData memory _table) {
